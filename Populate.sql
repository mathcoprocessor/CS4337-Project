...
SELECT movie.name, theater.name, showing.showtime 
    FROM (movie JOIN showing ON movie.id = showing.movieID) JOIN theater ON theater.ID = showing.THEATERID
    WHERE (theater.ZIPCODE = 75069)

-- Show Movies playing in a specified zipcode
SELECT DISTINCT movie.name, movie.ID
    FROM (movie JOIN showing ON movie.id = showing.movieID) JOIN theater ON theater.ID = showing.THEATERID
    WHERE (theater.ZIPCODE = 75034)

-- Show showtimes for a selected movie in a selected theater
SELECT showing.showtime
    FROM (movie JOIN showing ON movie.id = showing.movieID) JOIN theater ON theater.ID = showing.THEATERID
    WHERE (theater.ID = 4) AND (movie.ID = 4)